...
int block=16;//this is the block number we will write into and then read. Do not write into 'sector trailer' block, since this can make the block unusable.

byte blockcontent[16] = {"0"};//an array with 16 bytes to be written into one of the 64 card blocks is defined
